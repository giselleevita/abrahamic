 ...
 */
async function fetchChapter(book: string, chapter: number): Promise<ApiVerse[]> {
  const url = `${API}/${encodeURIComponent(book)}+${chapter}?translation=web`
  ...
  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt += 1) {
    ...
    if (data.error) throw new Error(`${book} ${chapter}: ${data.error}`)
    return data.verses ?? []
  }
